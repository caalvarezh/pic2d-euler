...
using namespace std;
namespace pic_cuda {
  const int MAX_SPE     = 1000;           // Limite (computacional) de Superpartículas electrónicas
  ...
  void H_poisson2D_dirichletX_periodicY(double *phi, complex<double> *rho, double hx) {
    int M = J_X - 2, N = J_Y;
    double h = hx;
    double hy = hx;
    double *f;
