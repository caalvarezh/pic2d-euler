...
//  const int Y           = 1;
  ...
  void H_electric_field (double *h_phi, double *h_E_X, double *h_E_Y, double hx) {
    ...
    cl::Buffer d_E_X, d_E_Y, d_phi;
    d_E_X = cl::Buffer(context, CL_MEM_WRITE_ONLY, size);
    d_E_y = cl::Buffer(context, CL_MEM_WRITE_ONLY, size);
    d_phi = cl::Buffer(context, CL_MEM_READ_ONLY, size);
    queue.enqueueWriteBuffer(d_phi, CL_TRUE, 0, bytes, h_phi);
/*
...
*/
    ...
    queue.enqueueWriteBuffer(d_E_X, CL_TRUE, 0, bytes, h_E_X);
    queue.enqueueWriteBuffer(d_E_Y, CL_TRUE, 0, bytes, h_E_Y);
    // free memory?
  }
