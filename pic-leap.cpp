...
void   Motion(double pos[MAX_SPE][2],  double vel[MAX_SPE][2],  int NSP,
              int especie, double E_X[J_X][J_Y], double E_Y[J_X][J_Y], int kt);//,
...
int main() {
  ...
  for (int kk  =  0, kt  =  0;kt<= K_total;kt++) {
    ...
    Motion(pos_e,vel_e,le, ELECTRONS, E_X, E_Y, kt);//, total_elec_perdidos, total_ion_perdidos, mv2_perdidas);
    Motion(pos_i,vel_i,li, IONS, E_X, E_Y, kt);//, total_elec_perdidos, total_ion_perdidos, mv2_perdidas);
